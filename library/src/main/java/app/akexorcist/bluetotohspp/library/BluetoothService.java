        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    if (flagCount == 0) {
                        if (data == 0x03) {
                           System.out.println("data == 0x03");
                            flagCount = 1;  // Next Check
                        }
                        arr_byte.add(data);
                    }
                    else if (flagCount == 1) {
                        if (data == 0x0A) {
                            System.out.println("data == 0x0A");
                            flagCount++;
                        } else if ( data == 0x03) {
                            flagCount = 1;
                        }
...
//                            System.out.println("Finish byte = " + buffer[buffer.length - 1]);
                            // Send the obtained bytes to the UI Activity
                            mHandler.obtainMessage(BluetoothState.MESSAGE_READ
                                    , buffer.length - 1, -1, buffer).sendToTarget();
